                ...
                <div class="rounded-[2rem] bg-white p-5 shadow-xl ring-1 ring-stone-100 md:p-8">
                    <div class="flex gap-3 overflow-x-auto border-b border-stone-100 pb-8 md:grid md:grid-cols-3 md:overflow-visible">
                        @foreach([
                            ...
                            <button type="button" class="js-step-indicator min-w-[210px] md:min-w-0 flex items-center gap-3 rounded-2xl border border-stone-200 px-4 py-4 text-left transition" data-step="{{ $index }}">
                                <span class="flex h-10 w-10 items-center justify-center rounded-2xl bg-stone-100 text-sm font-semibold text-gray-600">{{ $index }}</span>
                                <div>
                                    <p class="text-xs uppercase tracking-[0.2em] text-gray-400">Step {{ $index }}</p>
                    ...
                    <form method="POST" action="{{ route('bookings.store') }}" id="bookingWizardForm" class="pt-8">
                        @csrf
                        <input type="hidden" name="service_id" id="service_id" value="{{ $selectedServiceId ?: '' }}">
                        <input type="hidden" name="services" id="services_payload" value="{{ old('services') }}">
                        <input type="hidden" name="appointment_date" id="appointment_date" value="{{ $initialDate }}">
                        <input type="hidden" name="appointment_time" id="appointment_time" value="{{ $initialTime }}">
                        ...
                        <div class="space-y-8">
                            ...
                            <section class="js-step-panel hidden" data-step-panel="2">
                                ...
                                @include('partials.service-selector', [
                            ...
                            <section class="js-step-panel hidden" data-step-panel="3">
                                ...
                                <div class="grid gap-6 md:grid-cols-2">
                                    <div>
                                        ...
                                        <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full rounded-2xl border border-stone-300 px-4 py-3 focus:border-olive focus:outline-none focus:ring-2 focus:ring-olive/20">
                                    </div>
                                    <div>
                                        <label for="email" class="mb-2 block text-sm font-medium text-gray-700">Email address *</label>
                                        <input type="email" name="email" id="email" value="{{ old('email') }}" required class="w-full rounded-2xl border border-stone-300 px-4 py-3 focus:border-olive focus:outline-none focus:ring-2 focus:ring-olive/20">
                                    </div>
                                    <div>
                                        <label for="phone" class="mb-2 block text-sm font-medium text-gray-700">Phone number *</label>
                                        <input type="tel" name="phone" id="phone" value="{{ old('phone') }}" required class="w-full rounded-2xl border border-stone-300 px-4 py-3 focus:border-olive focus:outline-none focus:ring-2 focus:ring-olive/20">
                                    </div>
                                    <div>
                                ...
                                <div class="mt-6">
                                    ...
                                    <textarea name="notes" id="notes" rows="4" class="w-full rounded-2xl border border-stone-300 px-4 py-3 focus:border-olive focus:outline-none focus:ring-2 focus:ring-olive/20">{{ old('notes') }}</textarea>
                                </div>

                                <div class="mt-6 rounded-[2rem] border border-stone-200 bg-stone-50 p-6">
                                    <div class="flex items-start gap-3">
                                        <input type="checkbox" name="terms" id="terms" value="1" {{ old('terms') ? 'checked' : '' }} required class="mt-1 h-4 w-4 rounded border-stone-300 text-olive focus:ring-olive">
                                        <label for="terms" class="text-sm text-gray-700">I agree to the <a href="{{ route('policies') }}" class="font-medium text-olive hover:underline">Terms & Conditions</a> and <a href="{{ route('policies') }}" class="font-medium text-olive hover:underline">Privacy Policy</a>. A €15 deposit is required to secure the appointment; the remaining balance is paid after the service.</label>
                                    </div>
                                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const availableDates = @json(array_values($availableDates ?? []));
    ...
    if (!state.date && availableDates.length) {
        const requestedDate = @json($initialDate);
