...
@section('title', 'Edit Available Date')
...
<div class="min-h-screen bg-gradient-to-br from-beige-50 via-white to-olive-50 py-10">
    <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        <div class="rounded-[2rem] bg-white p-6 shadow-xl ring-1 ring-stone-100 md:p-8">
            ...
            <form method="POST" action="{{ route('admin.available-dates.update', $availableDate) }}" class="mt-8 space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="date" class="mb-2 block text-sm font-medium text-gray-700">Date</label>
                    <input type="date" name="date" id="date" value="{{ old('date', $availableDate->date->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4"
                              ...
                              placeholder="Special opening notes, breaks, or comments for this day...">{{ old('notes', $availableDate->notes) }}</textarea>
                    @error('notes')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="flex flex-col gap-3 sm:flex-row sm:justify-between">
                    <a href="{{ route('admin.available-dates.index', ['month' => $availableDate->date->format('Y-m'), 'date' => $availableDate->date->format('Y-m-d')]) }}" class="inline-flex items-center justify-center rounded-2xl border border-stone-200 px-5 py-3 font-semibold text-gray-700 transition hover:border-olive hover:text-olive">Cancel</a>
