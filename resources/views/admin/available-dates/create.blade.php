...
@section('title', 'Agregar Fecha Disponible')
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="px-6 py-4 bg-gray-800 text-white">
                <h1 class="text-2xl font-bold">Agregar Fecha Disponible</h1>
            </div>

            <form method="POST" action="{{ route('admin.available-dates.store') }}" class="p-6">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select name="slot_duration" id="slot_duration"
                            ...
                            required>
                        <option value="15" {{ old('slot_duration') == '15' ? 'selected' : '' }}>15 minutos</option>
                        <option value="30" {{ old('slot_duration') == '30' ? 'selected' : '' }}>30 minutos</option>
                        <option value="45" {{ old('slot_duration') == '45' ? 'selected' : '' }}>45 minutos</option>
                        <option value="60" {{ old('slot_duration') == '60' ? 'selected' : '' }}>1 hora</option>
                        <option value="90" {{ old('slot_duration') == '90' ? 'selected' : '' }}>1.5 horas</option>
                        <option value="120" {{ old('slot_duration') == '120' ? 'selected' : '' }}>2 horas</option>
                    </select>
                    @error('slot_duration')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex justify-between">
                    ...
                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium transition duration-200">
                        <i class="fas fa-save mr-2"></i>Guardar Fecha
                    </button>
                </div>
            </form>
        </div>
